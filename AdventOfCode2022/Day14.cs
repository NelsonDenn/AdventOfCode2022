using System;
using System.Collections.Generic;
using System.Linq;

namespace AdventOfCode2022
{
	public class Day14
	{
		private static readonly bool _isTest = false;

		public static int Run()
		{
			var data = _isTest ? GetTestData() : GetData();

			//var result = RunPart1(data);
			var result = RunPart2(data);

			return result;
		}

		private static int RunPart2(List<Point> points)
		{
			int maxY = points.Max(point => point.Y);

			// Add more points to create a floor
			int startX = _isTest ? 480 : 0;
			int endX = _isTest ? 520 : 1000;
			for (int x = startX; x <= endX; x++)
			{
				var floorPoint = new Point
				{
					X = x,
					Y = maxY + 2
				};

				points.Add(floorPoint);
			}

			return CountUnitsOfRestingSand(points);
			// Test: 93
			// Actual: 25055
		}

		private static int RunPart1(List<Point> points)
		{
			return CountUnitsOfRestingSand(points);
			// Test: 24
			// Actual: 1061
		}

		private static int CountUnitsOfRestingSand(List<Point> points)
		{
			int maxY = points.Max(point => point.Y);

			int restingUnitsOfSandCount = 0;

			int sourceX = 500;
			int sourceY = 0;

			// New units of sand always start at (500, 0)
			var sand = new Point
			{
				X = sourceX,
				Y = sourceY
			};

			// Sand continues falling into positions where they rest
			// until they start falling past the lowest point
			while (sand.Y <= maxY)
			{
				// A unit of sand always falls down one step if possible.
				if (!points.Any(point => point.X == sand.X && point.Y == sand.Y + 1))
				{
					sand.Y++;
				}
				// If the tile immediately below is blocked (by rock or sand),
				// the unit of sand attempts to instead move diagonally one step down and to the left.
				else if (!points.Any(point => point.X == sand.X - 1 && point.Y == sand.Y + 1))
				{
					sand.X--;
					sand.Y++;
				}
				// If that tile is blocked, the unit of sand attempts
				// to instead move diagonally one step down and to the right.
				else if (!points.Any(point => point.X == sand.X + 1 && point.Y == sand.Y + 1))
				{
					sand.X++;
					sand.Y++;
				}
				// If all three possible destinations are blocked,
				// the unit of sand comes to rest and no longer moves.
				else
				{
					points.Add(sand);
					restingUnitsOfSandCount++;

					// If the unit of sand is still at the source point, then we're done.
					if (sand.X == sourceX && sand.Y == sourceY)
					{
						break;
					}

					// Create the next unit of sand back at the source point.
					sand = new Point
					{
						X = sourceX,
						Y = sourceY
					};
				}

				//DrawCave(points, sand);
			}

			return restingUnitsOfSandCount;
		}

		private static void DrawCave(List<Point> points, Point sand)
		{
			int minX = points.Min(point => point.X);
			int maxX = points.Max(point => point.X);
			int minY = 0;
			int maxY = points.Max(point => point.Y);

			for (int y = minY; y <= maxY; y++)
			{
				for (int x = minX; x <= maxX; x++)
				{
					if (sand.X == x && sand.Y == y)
					{
						Console.Write("O");
					}
					else if (points.Any(point => point.X == x && point.Y == y))
					{
						Console.Write("#");
					}
					else
					{
						Console.Write(".");
					}
				}

				Console.WriteLine();
			}

			Console.WriteLine();
		}

		private static List<Point> GetTestData()
		{
			string data =
@"498,4 -> 498,6 -> 496,6
503,4 -> 502,4 -> 502,9 -> 494,9";

			return ParseData(data);
		}

		private static List<Point> GetData()
		{
			string data =
@"498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
481,79 -> 485,79
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
493,85 -> 497,85
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
499,85 -> 503,85
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
482,107 -> 487,107
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
496,82 -> 500,82
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
489,148 -> 489,149 -> 501,149 -> 501,148
482,49 -> 487,49
488,24 -> 488,25 -> 499,25 -> 499,24
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
489,148 -> 489,149 -> 501,149 -> 501,148
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,82 -> 482,82
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
493,79 -> 497,79
471,52 -> 476,52
469,58 -> 474,58
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
469,103 -> 469,104 -> 484,104 -> 484,103
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
479,29 -> 489,29 -> 489,28
475,85 -> 479,85
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
490,76 -> 494,76
486,109 -> 491,109
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
489,49 -> 494,49
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
488,24 -> 488,25 -> 499,25 -> 499,24
476,58 -> 481,58
476,111 -> 481,111
490,82 -> 494,82
483,111 -> 488,111
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
488,24 -> 488,25 -> 499,25 -> 499,24
479,56 -> 484,56
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
473,113 -> 478,113
480,113 -> 485,113
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
481,85 -> 485,85
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
490,111 -> 495,111
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
468,54 -> 473,54
484,76 -> 488,76
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
483,58 -> 488,58
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
465,56 -> 470,56
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
481,45 -> 486,45
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
494,113 -> 499,113
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
489,148 -> 489,149 -> 501,149 -> 501,148
472,56 -> 477,56
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
475,54 -> 480,54
485,47 -> 490,47
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
484,82 -> 488,82
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
462,58 -> 467,58
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
487,113 -> 492,113
487,73 -> 491,73
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
505,129 -> 505,131 -> 504,131 -> 504,135 -> 517,135 -> 517,131 -> 509,131 -> 509,129
469,103 -> 469,104 -> 484,104 -> 484,103
487,79 -> 491,79
479,29 -> 489,29 -> 489,28
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
487,85 -> 491,85
475,49 -> 480,49
479,109 -> 484,109
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
469,103 -> 469,104 -> 484,104 -> 484,103
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
485,42 -> 485,41 -> 485,42 -> 487,42 -> 487,38 -> 487,42 -> 489,42 -> 489,32 -> 489,42 -> 491,42 -> 491,33 -> 491,42 -> 493,42 -> 493,34 -> 493,42
498,13 -> 498,16 -> 496,16 -> 496,20 -> 509,20 -> 509,16 -> 502,16 -> 502,13
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
468,98 -> 468,96 -> 468,98 -> 470,98 -> 470,90 -> 470,98 -> 472,98 -> 472,90 -> 472,98 -> 474,98 -> 474,92 -> 474,98 -> 476,98 -> 476,97 -> 476,98 -> 478,98 -> 478,97 -> 478,98
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162
494,61 -> 494,63 -> 490,63 -> 490,70 -> 499,70 -> 499,63 -> 498,63 -> 498,61
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,47 -> 483,47
500,138 -> 500,142 -> 498,142 -> 498,146 -> 513,146 -> 513,142 -> 506,142 -> 506,138
493,126 -> 493,123 -> 493,126 -> 495,126 -> 495,117 -> 495,126 -> 497,126 -> 497,123 -> 497,126 -> 499,126 -> 499,122 -> 499,126 -> 501,126 -> 501,119 -> 501,126 -> 503,126 -> 503,125 -> 503,126 -> 505,126 -> 505,122 -> 505,126
478,162 -> 478,154 -> 478,162 -> 480,162 -> 480,159 -> 480,162 -> 482,162 -> 482,156 -> 482,162 -> 484,162 -> 484,161 -> 484,162 -> 486,162 -> 486,155 -> 486,162 -> 488,162 -> 488,159 -> 488,162 -> 490,162 -> 490,158 -> 490,162 -> 492,162 -> 492,159 -> 492,162 -> 494,162 -> 494,158 -> 494,162 -> 496,162 -> 496,153 -> 496,162";

			return ParseData(data);
		}

		private static List<Point> ParseData(string data)
		{
			var rows = data.Split("\r\n");

			var dataToReturn = new List<Point>();

			foreach (string row in rows)
			{
				var coordinates = row.Split(" -> ");
				Point previousPoint = null;

				foreach (var coordinate in coordinates)
				{
					var parts = coordinate.Split(',');

					// Get the current point represented by the coordinates
					var point = new Point
					{
						X = int.Parse(parts[0]),
						Y = int.Parse(parts[1])
					};
					dataToReturn.Add(point);

					if (previousPoint != null)
					{
						// Add a point for each coordinate between this point and the previous point
						if (point.X != previousPoint.X && point.Y != previousPoint.Y)
						{
							throw new Exception("I didn't account for this!");
						}
						else if (point.X != previousPoint.X)
						{
							int minX = Math.Min(point.X, previousPoint.X);
							int maxX = Math.Max(point.X, previousPoint.X);

							for (int x = minX + 1; x < maxX; x++)
							{
								var inBetweenPoint = new Point
								{
									X = x,
									Y = point.Y
								};
								dataToReturn.Add(inBetweenPoint);
							}
						}
						else if (point.Y != previousPoint.Y)
						{
							int minY = Math.Min(point.Y, previousPoint.Y);
							int maxY = Math.Max(point.Y, previousPoint.Y);

							for (int y = minY + 1; y < maxY; y++)
							{
								var inBetweenPoint = new Point
								{
									X = point.X,
									Y = y
								};
								dataToReturn.Add(inBetweenPoint);
							}
						}
						else
						{
							throw new Exception("I didn't account for this!");
						}
					}

					previousPoint = point;
				}
			}

			return dataToReturn;
		}

		class Point
		{
			public int X { get; set; }
			public int Y { get; set; }

			public override string ToString() => $"({X}, {Y})";
		}
	}
}
